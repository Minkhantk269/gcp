...
# =================== Telegram Push to Multiple Accounts ===================
TELEGRAM_TOKEN="${TELEGRAM_TOKEN:-}"
TELEGRAM_CHAT_ID1="${TELEGRAM_CHAT_ID1:-}"
TELEGRAM_CHAT_ID2="${TELEGRAM_CHAT_ID2:-}"
...
EOF
    ...
    for CHAT_ID in "${CHAT_IDS[@]}"; do
      curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_TOKEN}/sendMessage" \
        ...
        && ok "Telegram message sent to ${CHAT_ID}"
    done
  else
    warn "Telegram token set but no chat IDs provided"
  fi
else
  warn "Telegram not configured (set TELEGRAM_TOKEN via ENV or .env)"
fi
